...
7. **Back up automatically (v1.3).** After a successful add, commit and push the document store's own git repo (`backend/document-module/files/`, remote `https://github.com/itsabary13/butler-documents`) — `git add -A && git commit -m "..." && git push origin main`, run from inside `files/`. Do this by default, every time, same as `remember`'s v1.2 behavior. **Skip it only if the user's message for this add clearly asks to skip it** (e.g. "no push", "don't push this one") — applies to that single add only, not a standing preference. If the push fails, the add itself still stands — tell the user the local save succeeded but the backup push failed, don't fail the whole operation over it.
