...
- **Stores real files.** Give Jarvis a path to a file and ask it to save/store it, and it keeps an exact copy — no re-encoding, no compression, byte-for-byte.
...
- **Not backed up yet.** Unlike Memory (which auto-backs-up to a private repo), documents currently only live on this machine — the same risk Memory had before its backup was set up. Worth revisiting once you're storing real documents you'd mind losing.
...
- `backend/document-module/files/` — your stored documents (not committed to git — personal data, kept local only for now)
