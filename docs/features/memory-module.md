...
- **Structured reminders.** When you tell Jarvis something with both a date/recurrence and an action ("every 10th I need to deposit the rent check"), it's captured in a dedicated `reminders.md` list, not just as a regular memory.
- **Daily proactive digest (outside Claude Code).** A `claude.ai/code/routines` scheduled routine checks today's Google Calendar events plus a fixed reminder, and sends you a push notification once a day. It's set up separately from this repo's skills (no token/repo access from a routine was possible — see Known limitations) and doesn't automatically stay in sync with new entries added to `reminders.md`; update the routine's own instructions manually if the reminder changes.
...
- **The proactive digest lives outside this repo's skill system.** Claude Code's `routines` feature (as of this writing) can't attach private GitHub repos and has no environment-variable/secret storage, so the daily routine can't dynamically read `reminders.md` or use a Telegram bot the way originally planned — it uses only native connectors (Google Calendar) and native push notifications instead, with the reminder text itself hardcoded into the routine rather than fetched live.
