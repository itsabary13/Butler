...
- **The proactive digest lives outside this repo's skill system.** Claude Code's `routines` feature (as of this writing) can't attach private GitHub repos and has no environment-variable/secret storage, so it can't read `reminders.md` or use a Telegram bot directly — it relies entirely on native connectors (Google Calendar) and native push notifications instead. Push notifications from routines also turned out to require Remote Control paired to a machine — undocumented, but confirmed by testing.
