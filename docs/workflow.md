...
- **Never use real personal data as an illustrative example in anything destined for this public repo** (docs, skills, commit messages) — not an address, not a bank name, not a real family member's details, even as a "realistic-sounding" example. Everything under `.claude/skills/`, `docs/`, `specs/` is public. Use clearly fictional placeholders (a made-up name, "the storage unit invoice," "renew a document before it expires") instead. Real personal data belongs only in the gitignored `backend/memory-module/wiki/` and `backend/document-module/files/`, backed up solely to their private repos (`butler-memory`, `butler-documents`).
