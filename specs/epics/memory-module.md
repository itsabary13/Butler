...
**Actual v1.4 delivery**: a `claude.ai/code/routines` scheduled routine using only native, no-token features — the built-in **Google Calendar connector** (for today's events) and the routine's built-in **push notification on completion** (for delivery). Since the routine can't read `reminders.md` dynamically (no private-repo/token access), the routine's own instructions directly encode the current fixed reminder(s) as text — manually updated if a reminder ever changes, rather than automatically staying in sync with what `remember` writes to `reminders.md`. `reminders.md` itself remains the source of truth for what `remember` records; the routine is a manually-synced reflection of it, not a live reader.
