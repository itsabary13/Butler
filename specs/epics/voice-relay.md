...
**Status: Shipped**, plus one addendum pending live verification — live on the Phase 2 VPS (`jarvis.irenets.online`), Task 43's end-to-end verification against real credentials complete: voice and text conversation, memory save/recall, document upload, and Google Calendar event creation all confirmed working through the real deployment. See the v1.1–v1.5 addenda below for what changed since the original v1 design (local STT/TTS, VPS hosting, subscription billing instead of pay-per-token, text/document input). v1.6 (proactive notifications) is implemented and tested but ships disabled (`PROACTIVE_ENABLED=false`) pending its own manual live-verification pass — see that addendum.
